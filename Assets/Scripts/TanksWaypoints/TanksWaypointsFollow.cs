...
public class TanksWaypointsFollow : MonoBehaviour {
    ...
    // Ir ao helicóptero (Heli → WP05 → waypoints[4])
    ...
    // Ir á refinería (Refi → WP01 → waypoints[0])
    public void GotoRefi() {
        graph.AStar(currentNode, waypoints[0]);
        currentWP = 0;
    }


    // Ir á fábrica (Fábrica → WP03 → waypoints[2])
    ...
    // Ir ás ruínas (Ruin → WP04 → waypoints[3])
    ...
    // Ir á rocha (Roca → WP09 → waypoints[8])
